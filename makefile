...
containerID := $(shell docker ps -a -q --filter ancestor=mniyonshuti/kafka-test-server:latest)
...
#===================  docker  ===========================
docker-build: build
	docker build -t mniyonshuti/kafka-test-server:latest .


docker-run: docker-build
	docker run -p 8080:8080 mniyonshuti/kafka-test-server:latest
...
docker-push: docker-login docker-build
	docker push mniyonshuti/kafka-test-server:latest

docker-bash:
	docker exec -ti mniyonshuti/kafka-test-server:latest /bin/sh
...
k8s-persistence-create:
	...
	echo "kubectl run -it --rm --image=mysql:5.6 --restart=Never mysql-client -- mysql -h kafka-test-server-db -ppassword -e 'create database db_example;'"
...
k8s-info:
	...
	kubectl get pods
	echo "You can also check DB via:\n kubectl run -it --rm --image=mysql:5.6 --restart=Never mysql-client -- mysql -h kafka-test-server-db -ppassword "
